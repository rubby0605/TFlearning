...
/*  Test problem SCH1
    ...
    # of constraints = 0
    */
#ifdef getTransferFnc2 
#include "GetPrm.h"
double mat_t[numGrid], mat_Temp[numGrid];
void test_problem (double *xreal, double *xbin, int **gene, double *obj, double *constr)
{
	double ans, sum_diff, sum_diff2;
	int i;
	double X[7] = {0};

	sum_diff = 0;
	sum_diff2 = 0;
	X[0] = -(xreal[0] + xreal[1] + xreal[2] + xreal[3]) + mat_Temp[0];
	X[1] = xreal[1];
	X[2] = xreal[2];
	X[3] = xreal[3];
	X[4] = xreal[4];
	...
	for(i=0;i<numData;i++)
	{
		ans = getOptmVal(X, mat_t[i]*60);
		if(ans < mat_Temp[i] || mat_Temp[i] < 145)
		{
			sum_diff = sum_diff +  fabs(mat_Temp[i] - ans);
		}
		sum_diff2 = sum_diff2 + fabs(mat_Temp[i] - ans); 
	}
	obj[0] = sum_diff;
	obj[1] = sum_diff;
	return;
